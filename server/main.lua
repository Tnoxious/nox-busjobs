...
--Bonus pay
RegisterNetEvent(
    "nox-busjobcomplete:server:cPayBonus",
    function()
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
        if Player.PlayerData.job.name == "bus" then
...
RegisterNetEvent(
    "nox-busjobs:server:payDepositDash",
    function()
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
